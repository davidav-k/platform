...
| TD-010 | Medium | Gateway routing | The reserved `/api/tasks/**` gateway route rewrote traffic to singular `/api/v1/task/**`, while the future task-service contract uses plural `/api/v1/tasks/**`. | Gateway configuration and documentation now use the plural task-service contract path. | Resolved |
| TD-011 | Medium | Configuration | User-service runtime properties were duplicated between bundled `application.yml` and Config Server configuration, and tracked Docker hostnames obscured IDE startup. | Config Server now owns user-service runtime settings; local resources contain bootstrap metadata and safe developer defaults only. | Resolved |
