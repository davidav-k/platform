...
| task-service authorization is incomplete for planned write operations | Implemented read endpoints enforce admin and creator/assignee visibility, but planned update, delete, assign, status, and comment operations still need authorization rules | Medium | Apply explicit role and ownership checks when each planned write operation is implemented | Partially mitigated |
