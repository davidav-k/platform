...
import com.example.notification_service.outbox.OutboxEventPublisher;
...
public class KafkaOutboxEventPublisher implements OutboxEventPublisher {
    ...
    private final KafkaTemplate<String, String> notificationOutboxKafkaTemplate;
    ...
    public void publish(OutboxEventEntity event) {
        ...
        try {
            notificationOutboxKafkaTemplate.send(properties.getTopic(), key, value).get();
            log.info(
                    "Published notification outbox event to Kafka: eventId={}, eventType={}, aggregateType={}, aggregateId={}, topic={}",
                    event.getEventId(), event.getEventType(), event.getAggregateType(),
                    event.getAggregateId(), properties.getTopic()
            );
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Interrupted while publishing notification outbox event to Kafka", exception);
        } catch (ExecutionException exception) {
            throw new IllegalStateException("Failed to publish notification outbox event to Kafka", exception.getCause());
        }
    }
    ...
    private String serialize(KafkaOutboxEventMessage message) {
        try {
            ...
            throw new IllegalStateException("Failed to serialize notification outbox event for Kafka", exception);
        }
    }
}
