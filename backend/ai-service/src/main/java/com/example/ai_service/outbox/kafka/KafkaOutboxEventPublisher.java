...
import com.example.ai_service.outbox.OutboxEventPublisher;
...
public class KafkaOutboxEventPublisher implements OutboxEventPublisher {
    ...
    private final KafkaTemplate<String, String> aiOutboxKafkaTemplate;
    ...
    public void publish(OutboxEventEntity event) {
        ...
        try {
            aiOutboxKafkaTemplate.send(properties.getTopic(), key, value).get();
            log.info(
                    "Published AI outbox event to Kafka: eventId={}, eventType={}, aggregateType={}, aggregateId={}, topic={}",
                    event.getEventId(), event.getEventType(), event.getAggregateType(),
                    event.getAggregateId(), properties.getTopic()
            );
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new IllegalStateException("Interrupted while publishing AI outbox event to Kafka", exception);
        } catch (ExecutionException exception) {
            throw new IllegalStateException("Failed to publish AI outbox event to Kafka", exception.getCause());
        }
    }
    ...
    private String serialize(KafkaOutboxEventMessage message) {
        try {
            ...
            throw new IllegalStateException("Failed to serialize AI outbox event for Kafka", exception);
        }
    }
}
